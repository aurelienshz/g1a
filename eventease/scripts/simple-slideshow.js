    ...
    this.addInteractions = function() {
        ...
        for(var i=0, c=defilements.length; i<c; i++) {
            defilements[i].addEventListener('click', function(e) {
                ...
                if(e.target.className=='next' || e.target.parentNode.className=='next') {
                    currentSlideshow.next();
                }
                else if(e.target.className=='previous' || e.target.parentNode.className=='previous') {
...
(function() {
	var slideshows = document.getElementsByClassName('simple-slideshow');
