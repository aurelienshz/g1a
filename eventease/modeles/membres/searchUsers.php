<?php
function searchUsers($searchString, $criteres = []) {

    $keywords = explode(' ',strtolower($searchString));
    $bdd = new PDO(DSN, DBUSER, DBPASS);
    // squelette des requêtes sur les champs :
    $query = "SELECT membre.id, pseudo, media.lien FROM membre LEFT JOIN media ON membre.id_photo=media.id
                    WHERE ";
    ...
    $fields = [];
    if(!$criteres) {
        $fields = ['membre.pseudo'=>1,
                    'membre.mail'=>1];
    }
    else {
        if(in_array('pseudo',$criteres)) {
            $fields['membre.pseudo'] = 1;
        }
        if(in_array('email',$criteres)) {
            $fields['membre.mail'] = 1;
        }
    }
    ...
    foreach($fields as $field => $ponderation ) {
        ...
        foreach($keywords as $keyword) {
            if(!empty($keyword) && strlen($keyword)>3) {  // sanitizer : évite les doubles espaces et les lettres uniques
